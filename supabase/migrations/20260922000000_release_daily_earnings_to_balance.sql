-- Credit each daily earning to the user's available balance on its earning date.
-- The row lock/update guard makes retries idempotent and keeps withdrawals backed
-- by profiles.balance as the single source of truth.
...
BEGIN
  ...
  LOOP
    start_day := inv.start_day;

    FOR day_no IN 1..inv.duration_days LOOP
      IF start_day + (day_no - 1) > today_day THEN
        EXIT;
      END IF;
      ...
      VALUES (
        inv.id, inv.user_id, start_day + (day_no - 1), amount, 'pending', false
      )
